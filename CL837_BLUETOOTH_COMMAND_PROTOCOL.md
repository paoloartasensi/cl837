...
#### Comando 0x51 - Richiesta Temperatura
...
Future<void> requestTemperature() async {
  List<int> command = [
    0xFF,    // Start byte
    0x05,    // Length (4 + 1 parametro)
    0x51,    // Comando Temperatura
    0x00,    // Parametro
    0x00     // Checksum (calcolato)
  ];
  
  int sum = 0xFF + 0x05 + 0x51 + 0x00;
  int checksum = ((-sum) & 0xFF) ^ 0x3A;
  command[4] = checksum & 0xFF;
...
## Implementazione Attuale nel Progetto
...
void _processTemperatureData(List<int> data) {
  if (data.length >= 6 && data[2] == 0x51) {
    int tempRaw = (data[3] << 8) | data[4];
    double temperature = tempRaw / 100.0; // Conversione decimale
...
class CommandManager {
  ...
  Future<void> requestTemperature() => sendCommand(0x51, [0]);
  ...
  String _getCommandName(int command) {
    switch (command) {
      ...
      case 0x51: return 'Temperature Request';
  ...
  Duration _getCommandDelay(int command) {
    switch (command) {
      ...
      case 0x51: return Duration(milliseconds: 200);  // Temperature
      case 0x21:
